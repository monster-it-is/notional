...
- PostgreSQL is version 17, one replica, TLS on, public accessibility off. A runtime secret group scoped only to `notional-api` aliases provider `POSTGRES_URI` to `DATABASE_URL` when needed, and aliases `POSTGRES_URI_ADMIN` to `MIGRATION_DATABASE_URL` for the compiled migrator only. The long-running API uses `DATABASE_URL`. Builds and `notional-web` do not inherit either URI. The provider URIs are used unchanged. Do not add `DATABASE_SSL`, `NODE_TLS_REJECT_UNAUTHORIZED`, `rejectUnauthorized: false`, or a hand-written `sslmode`. If the current `pg` client cannot consume that TLS URI with verification left on, stop.

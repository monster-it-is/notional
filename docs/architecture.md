...
- Postgres: version 17, one replica, TLS on, public accessibility off. Runtime secret group for `notional-api` only. Alias `POSTGRES_URI` to `DATABASE_URL` if needed. Alias `POSTGRES_URI_ADMIN` to `MIGRATION_DATABASE_URL` for the API image entrypoint migrator only; the long-running API keeps `DATABASE_URL`. Do not inject either URI into builds or web, and do not add `DATABASE_SSL`, `NODE_TLS_REJECT_UNAUTHORIZED`, `rejectUnauthorized: false`, or a hand-written `sslmode`. If the generated TLS URI cannot be verified by `pg`, stop. Maintenance is addon port-forward.
